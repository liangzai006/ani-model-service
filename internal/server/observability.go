...
func (o *Observability) ServerMiddleware(logger *slog.Logger) []middleware.Middleware {
	return o.ServerMiddlewareWithPrincipal(logger, nil)
}

// ServerMiddlewareWithPrincipal adds the optional trusted-principal resolver
// to the standard Kratos middleware chain.
func (o *Observability) ServerMiddlewareWithPrincipal(logger *slog.Logger, resolver identity.Resolver) []middleware.Middleware {
	return []middleware.Middleware{
		recovery.Recovery(recovery.WithLogger(logger)),
		metadata.Server(),
		PrincipalMiddleware(resolver),
